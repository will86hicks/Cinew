...
<?php
	...
	$result = mysql_query("SELECT * FROM reservation WHERE id = {$resId}");
	$array = mysql_fetch_array($result);
	
	$cinemaID = $array['cinplex'];
	$theater = $array['theater'];
	$showtime = $array['date_time'];
	$title = $array['movie'];
		
	
	$movieIDResult= mysql_query("SELECT id FROM movie WHERE title = '{$title}'");
	$movieIDArray = mysql_fetch_array($movieIDResult);
	$movieID = $movieIDArray[0];
	
		
	$cinplexResult= mysql_query("SELECT name FROM cinplex WHERE id = {$cinemaID}");
	$cinplexArray = mysql_fetch_array($cinplexResult);
	$cinema = $cinplexArray[0];
